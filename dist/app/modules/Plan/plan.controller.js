"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PlanController = void 0;
const catchAsync_1 = __importDefault(require("../../../shared/catchAsync"));
const sendResponse_1 = __importDefault(require("../../../shared/sendResponse"));
const plan_service_1 = require("./plan.service");
const createPlan = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield plan_service_1.PlanService.createPlanIntoDb(req.body);
    (0, sendResponse_1.default)(res, {
        message: "Plan created successfully!",
        data: result,
    });
}));
const getPlans = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield plan_service_1.PlanService.getPlansFromDb();
    (0, sendResponse_1.default)(res, {
        message: "Plans retrieved successfully!",
        data: result,
    });
}));
const getSinglePlan = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield plan_service_1.PlanService.getSinglePlan(req.params.id);
    (0, sendResponse_1.default)(res, {
        message: "Plan retrieved successfully",
        data: result,
    });
}));
const updatePlan = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield plan_service_1.PlanService.updatePlan(req.body, req.params.id);
    (0, sendResponse_1.default)(res, {
        message: "Plan updated successfully!",
        data: result,
    });
}));
const deletePlan = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield plan_service_1.PlanService.deletePlan(req.params.id);
    (0, sendResponse_1.default)(res, {
        message: "Plan deleted successfully",
        data: result,
    });
}));
exports.PlanController = {
    createPlan,
    getPlans,
    getSinglePlan,
    updatePlan,
    deletePlan,
};
